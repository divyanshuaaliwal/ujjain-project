...
export default function Home() {
  ...
  return (
    <div className="container">
      <Head>
        <title>Mahakaleshwar Helicopter Tours</title>
      </Head>
      <h1 className="heading">Mahakaleshwar Yatra Tour Helicopter Packages</h1>
      <div className="card-container">
        {packages.map((pkg, index) => (
          <div className="card" key={index}>
            <Image src={pkg.image} className="card-image" alt={pkg.title} />
            <div className="card-body">
              <h3>{pkg.title}</h3>
              <p><span>🕒</span> {pkg.duration}</p>
              <p><span>📍</span> {pkg.route}</p>
              <p className="price"><strong>{pkg.price}</strong> <span className="old-price">{pkg.oldPrice}</span> per person</p>
