import package1 from "../images/package1.jpeg"
import package2 from "../images/package2.jpeg"
import package3 from "../images/package3.png"
import package4 from "../images/package4.jpeg"

import package5 from "../images/package1.jpeg"
import package6 from "../images/package2.jpeg"
import package7 from "../images/package3.png"
import package8 from "../images/package4.jpeg"

import package9 from "../images/package1.jpeg"
import package10 from "../images/package2.jpeg"
import package11 from "../images/package3.png"
import package12 from "../images/package4.jpeg"

import package13 from "../images/package1.jpeg"
import package14 from "../images/package2.jpeg"
import package15 from "../images/package3.png"
import package16 from "../images/package4.jpeg"

export const tourPackages = [
    {
        title: "Mahakal Darshan Package",
        location: "mahakaleshwar",
        img: package1,
        duration: "1 Day",
        price: "₹999",
        originalPrice: "₹1,299",
        rating: 4.7,
        reviews: 128,
        highlights: [
            "Bhasma Aarti experience",
            "Professional guide",
            "Complimentary breakfast",
            "Fast-track temple entry"
        ],
        popular: true
    },
    {
        title: "Ujjain Spiritual Tour",
        location: "mahakaleshwar",
        img: package2,
        duration: "2 Days / 1 Night",
        price: "₹2,999",
        originalPrice: "₹3,499",
        rating: 4.9,
        reviews: 86,
        highlights: [
            "Comfortable hotel accommodation",
            "All meals included",
            "Visit to 7 sacred temples",
            "Air-conditioned transportation"
        ],
        featured: true
    },
    {
        title: "Complete Ujjain Darshan",
        location: "mahakaleshwar",
        img: package3,
        duration: "3 Days / 2 Nights",
        price: "₹4,999",
        originalPrice: "₹5,999",
        rating: 4.8,
        reviews: 64,
        highlights: [
            "Premium hotel stay",
            "All inclusive meals",
            "All major attractions covered",
            "Complimentary pooja items"
        ]
    },
    {
        title: "Ujjain & Omkareshwar Jyotirling Yatra",
        location: "mahakaleshwar",
        img: package4,
        duration: "3 Days / 2 Nights",
        price: "₹5,499",
        originalPrice: "₹6,499",
        rating: 4.9,
        reviews: 102,
        highlights: [
            "Darshan of Mahakaleshwar & Omkareshwar Jyotirlingas",
            "Deluxe hotel stay",
            "Private cab transfers",
            "Evening Narmada Aarti experience"
        ],
        popular: true,
        featured: true
    },

    // Harsiddhi Mata Packages
    {
        title: "Harsiddhi Mata Darshan",
        location: "harsiddhi-mata",
        img: package5,
        duration: "1 Day",
        price: "₹799",
        originalPrice: "₹999",
        rating: 4.6,
        reviews: 94,
        highlights: [
            "Priority temple entry",
            "Local guide",
            "Prasad included",
            "Evening Aarti experience"
        ],
        popular: true
    },
    {
        title: "Mata Mandir Special Tour",
        location: "harsiddhi-mata",
        img: package6,
        duration: "2 Days / 1 Night",
        price: "₹2,499",
        originalPrice: "₹2,999",
        rating: 4.7,
        reviews: 78,
        highlights: [
            "Stay near Harsiddhi Temple",
            "Aarti + pooja samagri",
            "Spiritual walk",
            "Local shopping tour"
        ]
    },
    {
        title: "Divine Devi Circuit",
        location: "harsiddhi-mata",
        img: package7,
        duration: "2 Days / 1 Night",
        price: "₹3,499",
        originalPrice: "₹4,199",
        rating: 4.8,
        reviews: 52,
        highlights: [
            "Visit multiple Devi temples",
            "Deluxe stay + all meals",
            "Dedicated driver",
            "Temple history sessions"
        ]
    },
    {
        title: "Navratri Special Pooja Trip",
        location: "harsiddhi-mata",
        img: package8,
        duration: "1 Day",
        price: "₹1,199",
        originalPrice: "₹1,499",
        rating: 4.5,
        reviews: 60,
        highlights: [
            "Navratri pooja guided tour",
            "Prasad & pooja thali",
            "Cultural evening",
            "Fast-track temple access"
        ],
        featured: true
    },

    // Kal Bhairav Packages
    {
        title: "Kal Bhairav Temple Tour",
        location: "kal-bhairav",
        img: package9,
        duration: "1 Day",
        price: "₹899",
        originalPrice: "₹1,099",
        rating: 4.6,
        reviews: 112,
        highlights: [
            "VIP temple entry",
            "Liquor offering ritual",
            "Complimentary snacks",
            "Local guide"
        ],
        popular: true
    },
    {
        title: "Bhairav Bhakti Yatra",
        location: "kal-bhairav",
        img: package10,
        duration: "2 Days / 1 Night",
        price: "₹2,199",
        originalPrice: "₹2,799",
        rating: 4.7,
        reviews: 73,
        highlights: [
            "All-inclusive stay & meals",
            "Liquor & Prasad arrangement",
            "Personalized pooja service",
            "Transport to Bhairav temple"
        ]
    },
    {
        title: "Mystical Kal Bhairav Experience",
        location: "kal-bhairav",
        img: package11,
        duration: "2 Days / 1 Night",
        price: "₹2,999",
        originalPrice: "₹3,599",
        rating: 4.8,
        reviews: 64,
        highlights: [
            "Temple legends walkthrough",
            "Pooja items kit included",
            "Evening special ritual",
            "Deluxe accommodation"
        ]
    },
    {
        title: "Kal Bhairav Ritual Focus Tour",
        location: "kal-bhairav",
        img: package12,
        duration: "1 Day",
        price: "₹1,299",
        originalPrice: "₹1,599",
        rating: 4.5,
        reviews: 48,
        highlights: [
            "Liquor offering pooja",
            "Bhairav chantings & bhajans",
            "Light refreshments",
            "Spiritual souvenir included"
        ]
    },

    // Mangalnath Mandir Packages
    {
        title: "Mangalnath Mandir Darshan",
        location: "mangalnath-mandir",
        img: package13,
        duration: "1 Day",
        price: "₹899",
        originalPrice: "₹1,199",
        rating: 4.6,
        reviews: 85,
        highlights: [
            "Mangal Dosh nivaran pooja",
            "Priority access",
            "Prasad + astrologer session",
            "Cab pick-up available"
        ]
    },
    {
        title: "Mangal Dosh Shanti Package",
        location: "mangalnath-mandir",
        img: package14,
        duration: "2 Days / 1 Night",
        price: "₹2,799",
        originalPrice: "₹3,299",
        rating: 4.8,
        reviews: 72,
        highlights: [
            "Personal astrologer support",
            "Stay near temple",
            "Custom pooja arrangements",
            "Evening star-gazing session"
        ],
        featured: true
    },
    {
        title: "Mars Planet Energy Trip",
        location: "mangalnath-mandir",
        img: package15,
        duration: "2 Days / 1 Night",
        price: "₹3,199",
        originalPrice: "₹3,799",
        rating: 4.7,
        reviews: 55,
        highlights: [
            "Astrological guidance session",
            "Special mangal stotra path",
            "Group meditation",
            "Temple darshan with guide"
        ]
    },
    {
        title: "Mangalnath Pooja & Planet Walk",
        location: "mangalnath-mandir",
        img: package16,
        duration: "1 Day",
        price: "₹1,299",
        originalPrice: "₹1,699",
        rating: 4.5,
        reviews: 43,
        highlights: [
            "Sunrise temple visit",
            "Mangal dosh nivaran vidhi",
            "Temple astrologer session",
            "Lunch included"
        ]
    }
];
